...
import com.ssm.pay.common.utils.encode.SsmEncodeUtils;
import com.ssm.pay.common.utils.mapper.SsmJsonMapper;

/**HTTP 与 	servlet工具类 
* @author Jiang.Li
* @version 2016年1月28日 下午10:17:49
*/
public class SsmWebUtils extends org.springframework.web.util.WebUtils{
	private static final Log logger = LogFactory.getLog(SsmWebUtils.class);
	...
	public static boolean checkAccetptGzip(HttpServletRequest request) {
		...
		if (SsmStringUtils.contains(acceptEncoding, "gzip")) {
			return true;
		} else {
			return false;
		}
    ...
    public static void setDownloadableHeader(HttpServletRequest request,HttpServletResponse response, String fileName) {
        try {
            if(SsmBrowserUtils.isIE(request)){
                response.setHeader("content-disposition","attachment;filename=\""+ SsmEncodeUtils.urlEncode(fileName)+ "\"");
            }else{
                //中文文件名支持
    ...
    public static String encodeHttpBasic(String userName, String password) {
        ...
        return "Basic " + SsmEncodeUtils.base64Encode(encode.getBytes());
    }
    ...
    private static HttpServletResponse initResponseHeader(HttpServletResponse response,final String contentType, final String... headers) {
        ...
        for (String header : headers) {
            ...
            if (SsmStringUtils.equalsIgnoreCase(headerName, HEADER_ENCODING)) {
                encoding = headerValue;
            } else if (SsmStringUtils.equalsIgnoreCase(headerName, HEADER_NOCACHE)) {
    ...
    public static void render(HttpServletResponse response,final String contentType, final Object data, final String... headers) {
        ...
        try {
        	SsmJsonMapper.getInstance().writeValue(response.getWriter(),data);
        } catch (IOException e) {
    ...
    public static void renderJsonp(final String callbackName,HttpServletResponse response,Object data, final String... headers){
        String result = SsmJsonMapper.nonDefaultMapper().toJsonP(callbackName, data);
