...
/** 
* @author Jiang.Li
* @version 2016年4月15日 上午10:00:56
*/
public class SsmExcelUtils {
	/**
	 * JavaBean转Map
	 * @param obj
	 * @return
	 ...
	 * @return
	 */
	public static SsmTableHeaderMetaData createTableHeader(List<String> list){
		SsmTableHeaderMetaData headMeta = new SsmTableHeaderMetaData();
	        for(String title : list){
	        	SsmTableColumn tc = new SsmTableColumn();
		        tc.setDisplay(title);
	 ...
	 * @return
	 */
	public static SsmTableHeaderMetaData createTableHeader(String[] titls){
		SsmTableHeaderMetaData headMeta = new SsmTableHeaderMetaData();
	 ...
	 * @return
	 */
	public static SsmTableHeaderMetaData createTableHeader(String[] parents,String[][] children){
		SsmTableHeaderMetaData headMeta = new SsmTableHeaderMetaData();
		SsmTableColumn parentColumn = null;
		SsmTableColumn sonColumn = null;
		for (int i = 0; i < parents.length; i++) {
			parentColumn = new SsmTableColumn();
			parentColumn.setDisplay(parents[i]);
			if (children != null && children[i] != null) {
				for (int j = 0; j < children[i].length; j++) {
					sonColumn = new SsmTableColumn();
	...
	public static SsmTableData createTableData(List list,SsmTableHeaderMetaData headMeta,String[] fields){

		SsmTableData td = new SsmTableData(headMeta);
		SsmTableDataRow row = null;
        if(list != null || list.size()>0){
        	if(list.get(0).getClass().isArray()){//数组类型
                for (Object obj : list){
                	row = new SsmTableDataRow(td);
