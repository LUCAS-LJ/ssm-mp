 ...
 * @param <PK> 主键类型 
* @author Jiang.Li
* @version 2016年1月29日 上午10:25:09
*/
public class SsmSimpleHibernateDao<T,PK extends Serializable> {
	 ...
	 */
	public SsmSimpleHibernateDao(final SessionFactory sessionFactory, final Class<T> entityClass) {
     ...
     * @return
     */
    public int update(String hql, SsmParameter parameter) {
	...
	public void delete(final T entity) {
		Assert.notNull(entity, "entity不能为空");
		SsmDelete delete = SsmReflectionUtils.getAnnotation(entityClass,SsmDelete.class);
		if (delete != null) {
			Object value = SsmConvertUtils.convertToObject(delete.value(), delete.type().getValue());
			SsmReflectionUtils.invokeSetter(entity, delete.propertyName(), value);
			update(entity);
		} else {
    ...
    public T getByHql(String qlString, SsmParameter parameter) {
    ...
    public <X> List<X> find(String hql, SsmParameter parameter) {
	 ...
	 * 执行原生sql.
	 */
	public void executeJdbcUpdate(String sql) throws SsmDaoException{
		Assert.hasText(sql, "sql不能为空");
		try {
	 ...
	 * 执行原生sql查询.
	 */
	public ResultSet executeJdbcQuery(String sql) throws SsmDaoException{
		Assert.hasText(sql, "sql不能为空");
		try {
	...
	protected Criteria setPageParameterToCriteria(final Criteria c,
		...
		if (SsmStringUtils.isNotBlank(orderBy) && SsmStringUtils.isNotBlank(order)) {
			String[] orderByArray = SsmStringUtils.split(orderBy, ',');
			String[] orderArray = SsmStringUtils.split(order, ',');

			Assert.isTrue(orderByArray.length == orderArray.length,
					"分页多重排序参数中,排序字段与排序方向的个数不相等");

			for (int i = 0; i < orderByArray.length; i++) {
				if (SsmPage.ASC.equals(orderArray[i])) {
     ...
     * @return
     */
    public <E> List<E> findBySql(String sqlString, SsmParameter parameter) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <E> List<E> findBySql(String sqlString, SsmParameter parameter, Class<?> resultClass) {
     ...
     * @return
     */
    public int updateBySql(String sqlString, SsmParameter parameter) {
     ...
     * @return
     */
    public SQLQuery createSqlQuery(String sql, SsmParameter parameter) {
     ...
     * @return
     */
    public Query createQuery(String hql, SsmParameter parameter) {
     ...
     * @param parameter
     */
    protected void setParameter(Query query, SsmParameter parameter) {
        if (parameter != null) {
