...
/** 
* @author Jiang.Li
* @version 2016年4月15日 上午9:54:37
*/
public class SsmTableColumn {
	/**
	 * 字符串型
	 */
	...
	protected SsmTableColumn parent = null;

	/**
	 * 复合表头，拥有子列
	 */
	protected List<SsmTableColumn> children = new LinkedList<SsmTableColumn>();

	public void addChild(SsmTableColumn column) {
		children.add(column);
		column.parent = this;
		if (column.isVisible())
			isComplex = true;
	...
	public List<SsmTableColumn> getChildren() {
		LinkedList<SsmTableColumn> ret = new LinkedList<SsmTableColumn>();
		for (SsmTableColumn c : children) {
			if (c.isVisible())
				ret.add(c);
		}
		return ret;
	...
	public int getLength() {
		int len = 0;
		if (isComplex()) {
			for (SsmTableColumn col : getChildren()) {
				len += col.getLength();
			}
		} else {
			len = 1;
