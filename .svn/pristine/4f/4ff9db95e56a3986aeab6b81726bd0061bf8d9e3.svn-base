/*@版权所有LIJIANG*/ 
package com.ssm.pay.common.utils.mapper;
...
* @author Jiang.Li
* @version 2016年1月29日 上午11:16:12
*/
public class SsmJsonMapper extends ObjectMapper{
	private static Logger logger = LoggerFactory.getLogger(SsmJsonMapper.class);

	private static SsmJsonMapper mapper;


    public SsmJsonMapper() {
        this(null);
    }

    public SsmJsonMapper(Include include) {
		//设置输出时包含属性的风格
        if (include != null) {
...
//        });
     ...
     */
    public static SsmJsonMapper getInstance() {
        if (mapper == null){
            mapper = new SsmJsonMapper();
        }
        return mapper;
    }
     ...
     * 创建只输出非Null且非Empty(如List.isEmpty)的属性到Json字符串的Mapper,建议在外部接口中使用.
     */
    public static SsmJsonMapper nonEmptyMapper() {
        return new SsmJsonMapper(Include.NON_EMPTY);
    }

    /**
     * 创建只输出初始值被改变的属性到Json字符串的Mapper, 最节约的存储方式，建议在内部接口中使用。
     */
    public static SsmJsonMapper nonDefaultMapper() {
        return new SsmJsonMapper(Include.NON_DEFAULT);
    }
     ...
     * 允许不带引号的字段名称
     */
    public SsmJsonMapper enableSimple() {
     ...
     * @return
     */
    public SsmJsonMapper registerHibernate4Module(){
	...
	public <T> T fromJson(String jsonString, Class<T> clazz) {
		if (SsmStringUtils.isEmpty(jsonString)) {
			return null;
		}
	 ...
	 */
	public SsmJsonMapper enableEnumUseToString() {
	 ...
	 */
	public SsmJsonMapper enableJaxbAnnotation() {
