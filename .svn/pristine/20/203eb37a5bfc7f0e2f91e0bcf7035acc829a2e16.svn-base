/*@版权所有LIJIANG*/ 
package com.ssm.pay.common.utils.browser;
...
* 浏览器工具类 
* @author Jiang.Li
* @version 2016年1月28日 下午10:48:04
*/
public class SsmBrowserUtils {
     ...
     * @return
     */
    public static SsmBrowserType getBrowserType(HttpServletRequest request) {
        SsmBrowserType browserType = null;
        if (getBrowserType(request, IE11)) {
            browserType = SsmBrowserType.IE11;
        }
        if (getBrowserType(request, IE10)) {
            browserType = SsmBrowserType.IE10;
        }
        if (getBrowserType(request, IE9)) {
            browserType = SsmBrowserType.IE9;
        }
        if (getBrowserType(request, IE8)) {
            browserType = SsmBrowserType.IE8;
        }
        if (getBrowserType(request, IE7)) {
            browserType = SsmBrowserType.IE7;
        }
        if (getBrowserType(request, IE6)) {
            browserType = SsmBrowserType.IE6;
        }
        if (getBrowserType(request, FIREFOX)) {
            browserType = SsmBrowserType.Firefox;
        }
        if (getBrowserType(request, SAFARI)) {
            browserType = SsmBrowserType.Safari;
        }
        if (getBrowserType(request, CHROME)) {
            browserType = SsmBrowserType.Chrome;
        }
        if (getBrowserType(request, OPERA)) {
            browserType = SsmBrowserType.Opera;
        }
        if (getBrowserType(request, CAMINO)) {
            browserType = SsmBrowserType.Camino;
        }
        return browserType;
    }
