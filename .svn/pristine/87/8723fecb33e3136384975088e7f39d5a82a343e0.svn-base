...
/** 
* @author Jiang.Li
* @version 2016年4月15日 上午10:02:39
*/
public class SsmJsGridReportBase {
	...
	public SsmJsGridReportBase() {
	}

	public SsmJsGridReportBase(HttpServletRequest request,
	 ...
	 * @return void
	 */
	public void outDataToBrowser(SsmTableData tableData) {
		...
		SsmTableHeaderMetaData headerMetaData = tableData.getTableHeader();
		List<SsmTableDataRow> dataRows = tableData.getRows();
		try {
			for (SsmTableDataRow dataRow : dataRows) {
				List<SsmTableDataCell> dataCells = dataRow.getCells();
	 ...
	 * @return void
	 */
	private void generateColumn(HSSFSheet sheet, SsmTableColumn tc, int maxlevel,
	 ...
	 * @throws Exception
	 */
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, SsmTableData tableData) throws Exception {
		...
		SsmTableHeaderMetaData headerMetaData = tableData.getTableHeader();// 获得HTML的表头元素
        ...
        System.out.println(SsmJsonMapper.getInstance().toJson(headerMetaData));
        int colnum = 0;
		for (int i = 0; i < headerMetaData.getOriginColumns().size(); i++) {
			SsmTableColumn tc = headerMetaData.getOriginColumns().get(i);
			if (i != 0) {
		...
		for (SsmTableDataRow dataRow : dataRows) {
			...
			index = -1;
			for (int i = 0; i < size; i++) {
				SsmTableColumn tc = headerMetaData.getColumns().get(i);
				if (!tc.isVisible())
					continue;
				index++;
	 ...
	 */
	public void exportToExcel(String title, String creator, SsmTableData tableData)
			throws Exception {
		...
		SsmWebUtils.setDownloadableHeader(request,response,sFileName);
		response.setHeader("Connection", "close");
		response.setHeader("Content-Type", SsmWebUtils.EXCEL_TYPE);
	 ...
	 *         bgcolor:#CAEAFE; </style>
	 */
	public void exportToExcel(String title, String creator, List<SsmTableData> tableDataLst)
		throws Exception {
		...
		int i = 1;
		for(SsmTableData tableData : tableDataLst){
	 ...
	 * @return void
	 */
	private void createCell(HSSFRow row, SsmTableColumn tc,
			List<SsmTableDataCell> data, int i, int index,
			HashMap<String, HSSFCellStyle> styles) {
		SsmTableDataCell dc = data.get(i);
		HSSFCell cell = row.createCell(index);
		switch (tc.getColumnType()) {
		case SsmTableColumn.COLUMN_TYPE_INTEGER:
			...
			if (style != null)
				cell.setCellStyle(style);
			break;
		case SsmTableColumn.COLUMN_TYPE_FLOAT_2:
			...
			if (style != null)
				cell.setCellStyle(style);
			break;
		case SsmTableColumn.COLUMN_TYPE_FLOAT_3:
			cell.setCellValue(dc.getDoubleValue());
			style = styles.get("D3");
			if (row.getRowNum() % 2 != 0)
			...
			if (style != null)
				cell.setCellStyle(style);
			break;
		case SsmTableColumn.COLUMN_TYPE_RED_BG:
			cell.setCellValue(dc.getValue());
			style = styles.get("RED_BG");
			if (style != null)
				cell.setCellStyle(style);
			break;
		case SsmTableColumn.COLUMN_TYPE_YELLOW_BG:
			cell.setCellValue(dc.getValue());
			style = styles.get("YELLOW_BG");
			if (style != null)
				cell.setCellStyle(style);
			break;
		case SsmTableColumn.COLUMN_TYPE_GREEN_BG:
			cell.setCellValue(dc.getValue());
	...
	private void buildStyle(HSSFWorkbook wb, HSSFWorkbook src, HSSFSheet sheet,
		...
		SsmExcelUtils.copyCellStyle(wb, nstyle, src, cell.getCellStyle());
		ret.put(key, nstyle);
	}
